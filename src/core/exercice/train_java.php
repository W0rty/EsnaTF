...
$sth_ex = $db->prepare("SELECT * FROM exercice_java WHERE numero = :n");
$sth_ex->bindValue(":n",htmlspecialchars($_GET["id"]));
$sth_ex->execute();
$res_ex = $sth_ex->fetchAll();
if($res_ex){
    $hash = hash("sha256",rand());
    $sth_encours = $db->prepare("INSERT INTO en_cours(id,users,exercice_python,exercice_java,exercice_c,token) VALUES (0,:u,null,:e,null,:t)");
    $sth_encours->bindValue(":u",$res_user[0][0]);
    $sth_encours->bindValue(":e",$res_ex[0][0]);
    $sth_encours->bindValue(":t",$hash);
    $sth_encours->execute();
    $java_solves = explode(",",$res_user[0][7]);
    if(substr_count($res_user[0][7],",") > 0){
        $enable = [];
        $disable = [];
        $isNext = False;
        foreach($java_solves as $solve)
        {
            if($solve === $_GET["id"]){
    ...
    <div class="wrapper">
        ...
        <nav id="sidebar">
            ...
            <ul class="list-unstyled CTAs">
                <li>
                    <a href="/exercice/java" class="download">Back to java</a>
                </li>
                <li>
