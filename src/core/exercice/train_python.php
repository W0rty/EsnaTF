...
if(!isset($_GET["id"])){
    header("Location: /exercice/python");
    die();
}
...
$sth_ex = $db->prepare("SELECT * FROM exercice_python WHERE numero = :n");
...
if($res_ex){
    ...
    $sth_encours = $db->prepare("INSERT INTO en_cours(id,users,exercice_python,exercice_java,exercice_c,token) VALUES (0,:u,:e,null,null,:t)");
    $sth_encours->bindValue(":u",$res_user[0][0]);
    $sth_encours->bindValue(":e",$res_ex[0][0]);
    $sth_encours->bindValue(":t",$hash);
    $sth_encours->execute();
    $python_solves = explode(",",$res_user[0][3]);
    if(substr_count($res_user[0][3],",") > 0){
        $enable = [];
        $disable = [];
        $isNext = False;
        foreach($python_solves as $solve)
        {
            if($solve === $_GET["id"]){
    ...
    <div class="wrapper">
        ...
        <nav id="sidebar">
            ...
            <ul class="list-unstyled CTAs">
                <li>
                    <a href="/exercice/python" class="download">Back to python</a>
                </li>
                <li>
