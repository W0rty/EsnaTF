...
$sth_ex = $db->prepare("SELECT numero,titre FROM exercice_c");
$sth_ex->execute();
$res_ex = $sth_ex->fetchAll();
$sth_all_ex = $db->prepare("SELECT COUNT(*) FROM exercice_c");
$sth_all_ex->execute();
$nb_all_ex = $sth_all_ex->fetchAll()[0][0];
$java_solves = explode(",",$res_user[0]["solved_c"]);
$solved = [];
$todo = [];
$disable = [];
$lastI = 0;
if($nb_all_ex != "0"){
    if(substr_count($res_user[0][8],",") === 0)
    {
        ...
        }else $message = "Bravo ! Vous avez complété tous les exercices en c ! <br> Vous pouvez passer à une autre catégorie :)";
    }
}else{
    $message = "Les exercices en c ne sont pas encore disponible, revenez plus tard!";
    $avancement = 100;
}
?>
<!DOCTYPE HTML>
...
<body>
    <div class="wrapper">
                    ...
                    <ul class="collapse list-unstyled" id="pageSubmenu">
                        ...
                        if(sizeof($solved) > 0){
                            foreach($solved as $solv){
                                echo('<li><a href="/exercice/c/train?id='.$solv[0].'" style="color: lightgreen;">'.$solv[1].'</a></li>');
                            }
                        }
                        if(sizeof($todo) > 0) echo ('<li><a id="todo" onmouseleave="$(\'#todo\').css(\'color\',\'white\')" onmouseover="$(\'#todo\').css(\'color\',\'black\')" href="/exercice/c/train?id='.$todo[0][0].'" style="color: white;">'.$todo[0][1].'</a></li>');
                        if(sizeof($disable) > 0){
        ...
        <div id="content">
            ...
            <h2>Partie Exercice - C</h2>
            <p>Vous êtes sur la partie C du site internet, avec des exercices, que vous trouverez dans le menu à côté.</p>

            <div class="line"></div>
            <label for="avancement">Avancement en C : </label>
