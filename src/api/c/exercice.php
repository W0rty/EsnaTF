...
if(isset($_POST["code"]) && isset($_POST["hash"]) && !empty($_POST["code"])){
    ...
    if($res_ec){
        $sth_ex = $db->prepare("SELECT * FROM exercice_c WHERE id = :id");
        $sth_ex->bindValue(":id",$res_ec[0][4]);
        $sth_ex->execute();
        $res_ex = $sth_ex->fetchAll();
        $valid_result = $res_ex[0][3];
        $real_code = base64_decode($_POST["code"]);
        $res = trim(create_secure_code_c($real_code,$res_ex[0][4]),"\x00..\x1F");
        if($res === $valid_result){
            $sth_reussi = $db->prepare("UPDATE users SET solved_c = :sp WHERE username = :u");
            $sth_reussi->bindValue(":u",htmlspecialchars($_SESSION["username"]));
            if(substr_count($res_user[0][8],",") === 0) $sth_reussi->bindValue(":sp",$res_ec[0][4].",");
            else $sth_reussi->bindValue(":sp",$res_user[0][8].$res_ec[0][4].",");
            $sth_reussi->execute();
